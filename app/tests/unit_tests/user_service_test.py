...
async def test_create_user(uow: UnitOfWork):
...
async def test_already_exist_user_create(uow: UnitOfWork):
    user_create = UserCreate(
...
async def test_login_user(uow: UnitOfWork):
    async with uow:
...
async def test_refresh_user_token(uow: UnitOfWork):
...
async def test_reset_user_password(uow: UnitOfWork):
...
async def test_fail_reset_user_password(uow: UnitOfWork):
